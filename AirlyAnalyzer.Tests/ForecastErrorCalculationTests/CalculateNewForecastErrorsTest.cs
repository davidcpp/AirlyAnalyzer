namespace AirlyAnalyzer.Tests
{
  ...
  public class CalculateNewForecastErrorsTest
  {
    ...
    [Fact]
    public void Return_correct_forecast_error_list_from_overlapping_elements()
    {
      // Arrange
    ...
    [Fact]
    public void Return_empty_forecast_errors_from_non_overlapping_measurements()
    {
      // Arrange
      const short installationId = 1;
    ...
    public void Return_negative_forecast_values_when_measurements_are_zero(
    ...
    public void Return_correctly_calculated_forecast_values(
      byte airlyCaqi_Measurement, short airlyPm25_Measurement, short airlyPm10_Measurement,
      byte airlyCaqi_Forecast, short airlyPm25_Forecast, short airlyPm10_Forecast,
      short airlyCaqi_ForecastError, short airlyPm25_ForecastError, short airlyPm10_ForecastError)
    {
    ...
    public void Return_correct_daily_forecast_error_when_number_of_elements_is_equal_min()
    {
      ...
      var newMeasurements = AuxiliaryMethods.GenerateMeasurements(
        _startDate, numberOfMeasurements, _requestMinutesOffset).ToList();

      var newForecasts = AuxiliaryMethods.GenerateForecasts(
        _startDate, numberOfForecasts, _requestMinutesOffset).ToList();
        ...
        short numberOfDailyForecastsErrors)
    {
      ...
      var newMeasurements = AuxiliaryMethods.GenerateMeasurements(
        _startDate, numberOfDays, numberOfMeasurementsInDay, _requestMinutesOffset).ToList();

      var newForecasts = AuxiliaryMethods.GenerateForecasts(
        _startDate, numberOfDays, numberOfForecastsInDay, _requestMinutesOffset).ToList();
